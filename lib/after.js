...
*/
function forEach(obj, iterator, context, callback) {
    ...
    var keys = Object.keys(obj),
        next = after(keys.length, callback)
    ...
    function loop(key) {
        ...
        invokeIterator(iterator, context, [value, key, obj], proxy)
...
function invokeIterator(iterator, context, params, callback) {
    params = params.slice(0, iterator.length - 1)
    params.push(callback)
    iterator.apply(context, params)
}
...
function flow(array, context, callback) {
    if (typeof context === "function") {
        callback = context
        context = null
    }
    array.reduce(function (memo, outer) {
        return function (inner) {
            memo.call(context, function (err) {
                if (err) {
                    return callback(err)
                }
